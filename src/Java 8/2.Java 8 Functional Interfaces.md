...
We mark functional interfaces using `@FunctionalInterface` — this ensures the compiler throws an error if more than one abstract method is added.
...
Java 8 introduced **Lambda Expressions** → which require a **target type** (Functional Interface).

💡 **Functional Interface acts as a bridge between OOP and Functional Programming**.
...
## 🧩 **4. Predefined Functional Interfaces in `java.util.function`**
...
“Predicate” is like a filter gate — only allows data that meets a condition to pass.
----------
...
“Function” is like a converter machine — takes input, applies logic, and gives output.
----------



### 3.  **Consumer<T>**  - The Action Performer ⚡
...
“Consumer” is like a speaker — it consumes your input and produces side effects (output/print).
----------
...
“Supplier” is like a vending machine — press the button, it gives something.
----------



### 5.  **BiFunction<T, U, R>**  - The Two-Input Transformer 🔀
...
### 6.  **BiConsumer<T, U>**  - The Two-Input Action Performer ⚡⚡
...
### 7.  **BiPredicate<T, U>**  - The Two-Input Tester 🔍🔍
...
### 8.  **UnaryOperator<T>**  - The Self-Transformer 🔁
...
### 9.  **BinaryOperator<T>**  - The Two-Input Self-Transformer 🔁🔁
...
HEAP MEMORY
┌─────────────────────────────────────────┐
│  Anonymous Object (Lambda Instance)     │
│  ┌────────────────────────────────┐    │
│  │  Functional Interface Type     │    │
│  │  ─────────────────────────     │    │
│  │  Method: apply/test/accept     │    │
│  │  Lambda Body: { code }         │    │
...
/* Memory Layout:
   ...
   STACK:
   ├─ multiplier = 10
   └─ func → [Reference to Lambda Object]
   
   HEAP:
   Lambda Object {
     type: Function<Integer, Integer>
     captured: [multiplier = 10]  ← Closure
     code: x -> x * multiplier
   }
*/
```
...
### 1.  **Predicate = Security Guard**  🚨

-   **Job:**  Check if you can enter (true/false)
-   **Example:**  "Are you 18+?" → Yes/No
...
### 2.  **Function = Vending Machine**  🏪

-   **Job:**  Take input (money), return output (product)
-   **Example:**  Insert ₹20 → Get Chips
...
### 3.  **Consumer = Printer**  🖨️

-   **Job:**  Take input, do something, return nothing
-   **Example:**  Take document → Print it (no return)
...
### 4.  **Supplier = ATM Machine**  🏧

-   **Job:**  No input needed, just give money
-   **Example:**  Press button → Get cash
...
### 6.  **UnaryOperator = Amplifier**  📢

-   **Job:**  Take input, modify it, return same type
-   **Example:**  Volume 5 → Volume 15 (amplified 3x)
...
-   ❌ When debugging is priority (lambdas harder to debug)
...
**Answer:**  
Yes. Default and static methods are allowed. Only one **abstract method** is allowed.

----------

### ❓Q3: What happens if you add two abstract methods in a `@FunctionalInterface`?

**Answer:**  
Compiler error: _“Invalid '@FunctionalInterface' annotation”_.

----------

### ❓Q4: How do Lambdas relate to Functional Interfaces?

**Answer:**  
Lambda expressions provide the **implementation** for the single abstract method of a Functional Interface.

----------

### ❓Q5: Write a Lambda that checks if a string length > 5.

**Answer:**
``` java

Predicate<String> check = s -> s.length() > 5;
System.out.println(check.test("SpringBoot")); // true` 
```
----------

### ❓Q6: Can we use method references with functional interfaces?
...
### ❓Q7: How does `and()`, `or()`, `negate()` work in Predicate?

**Answer:**  
They combine multiple predicates.
``` java
Predicate<Integer> even = n -> n % 2 == 0;
Predicate<Integer> gt5 = n -> n > 5;

Predicate<Integer> condition = even.and(gt5);
System.out.println(condition.test(8)); // true` 
```
----------

## 🧩 **11. Practice Interview Problems**

### ✅ Problem 1: Find names starting with “S” and length > 5
``` java
List<String> names = Arrays.asList("Sam", "Steve", "Suresh", "Reddy");
Predicate<String> startsWithS = n -> n.startsWith("S");
Predicate<String> lengthCheck = n -> n.length() > 5;

names.stream()
     .filter(startsWithS.and(lengthCheck))
     .forEach(System.out::println);` 
```
### ✅ Problem 2: Use Function to convert list of names to uppercase
``` java
Function<String, String> toUpper = String::toUpperCase;
names.stream().map(toUpper).forEach(System.out::println);` 
```
### ✅ Problem 3: Use Supplier to get random OTP
``` java
Supplier<String> otp = () -> String.valueOf((int)(Math.random()*9000 + 1000));
